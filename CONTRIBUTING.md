...
アサルトリリィRDFデータベースのデータ整備にご協力いただきありがとうございます。
...
- GitHub で PullRequest を送ったりできる方
  ...
  PullRequest を送ると、送っていただいたデータのフォーマットを rdflint というフォーマット検査ツールが検査しますので、合格するまで修正してください。(LuciaDB に PR を送るのが初めての場合、管理者が承認するまで rdflint による検査は走りません。気づき次第承認しますのでお待ちください。)
  
  最終的には管理人(ふぁぼ原)がデータをチェックし、取り込みます。
...
本リポジトリに対して提供いただくデータに関する一切の権利は、当該のデータが本リポジトリに収録された時点で `AZONE INTERNATIONAL・acus` に帰属します。本リポジトリは全てのデータを `AZONE INTERNATIONAL・acus` を原著作者として [CC BY-NC-SA 4.0 国際 ライセンス](https://creativecommons.org/licenses/by-nc-sa/4.0/deed.ja)のもとに公開します。

LuciaDBは著作者の許可を得て運営しています。
